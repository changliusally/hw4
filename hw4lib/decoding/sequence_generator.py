...
class SequenceGenerator:
    ...
    def generate_greedy(
        ...
        # generated_tokens = [x]  # Store all generated tokens, including the initial input
        ...
        for _ in range(self.max_length - x.size(1)):
            ...
            # repeat penalty
            logits = self._apply_repeat_penalty(logits, x, penalty=repeat_penalty)  # Assuming this function is defined
            ...
            # Compute log-probs for selected token
            ...
            # Append next tokens
            x = torch.cat([x, next_tokens.unsqueeze(1)], dim=1)  # (batch_size, sequence_length + 1)
    ...
    def generate_beam(
        ...
        # sequences: (batch_size, beam_width, sequence_length)
        ...
        logits = self.score_fn(x)  # (batch_size, vocab_size)
        logits = self._apply_repeat_penalty(logits, x, repeat_penalty)  # Apply repeat penalty
        ...
        for _ in range(self.max_length - sequences.size(-1)):
            ...
            if eos_mask.any():
                ...
                log_probs = torch.where(eos_mask & (torch.arange(vocab_size,
                                        device=x.device).view(1, 1, -1) != eos_id),
            ...
            new_scores  = top_vals
            # Get beam index and token index
            beam_idx = top_idx // vocab_size # (batch_size, beam_widt
